        ...
        while True:
            ...
            if data:
                ...
                if re.match(b"[0-9]", data):
                    ...
                    ser.write(str.encode("#" + mhz + "." + ltmhz + "\r\n"))
                ...
                if (data == "M"):
                    ser.write(str.encode("!HAM\r\n"))
                if (data == "k"):
                    ser.write(str.encode("!MRN\r\n"))
                    current_state = "trunk1"
                    trunk1()
                if (data == "U"):
                    ser.write(str.encode("146\r\n"))
                if (data == "D"):
                    ser.write(str.encode("#146.520\r\n"))
                if (data == "r"):
                    nrsc5()
                if (data == "d"):
                    ...
                    ser.write(str.encode(octets[0].ljust(3)))
                    ser.write(str.encode('\r\n'))
                    time.sleep(1)
                    ser.write(str.encode(octets[1].ljust(3)))
                    ser.write(str.encode('\r\n'))
                    time.sleep(1)
                    ser.write(str.encode(octets[2].ljust(3)))
                    ser.write(str.encode('\r\n'))
                    time.sleep(1)
                    ser.write(str.encode(octets[3].ljust(3)))
                    ser.write(str.encode('\r\n'))
                    time.sleep(1)
                    ser.write(str.encode('@\r\n'))
...
def trunk1():
    ...
    time.sleep(1)
    with canvas(device) as draw:
        draw.text((10,40), "152.7250")
    #ser.write(str.encode("#152.7250\r\n"))
    time.sleep(1)
    ser.write(str.encode("!WX\r\n"))
    if (PID):
        os.kill(PID, signal.SIGTERM)
...
def on_message(client, userdata, message):
    ...
    if re.match(b".+NAC 0x230", message.payload):
...
def show_last_call(radio_id):
    ...
    if dict.get(radio_id):
        ...
        ser.write(str.encode(dict.get(radio_id) + "\r\n"))
    else:
        print("unknown RID:" + radio_id + ":")
        ser.write(str.encode("UNK\r\n"))
